...
OEMER_SUPPORTED_EXT = SUPPORTED_IMAGE_EXT | {".pdf"}
...
async def parse_drumsheet(
    file: UploadFile = File(...), bpm: Optional[int] = 100, engine: str = "auto"
):
    ...
    try:
        ...
        if engine not in VALID_ENGINES:
            raise HTTPException(
                status_code=400,
                detail=f"Invalid engine '{engine}'. Choose from: {', '.join(sorted(VALID_ENGINES))}.",
            )

        is_musicxml = ext in (".xml", ".musicxml")
        is_supported_image = _is_supported_image(ext)
        can_use_detector = bool(INFERENCE_RUNNER) and is_supported_image
        can_use_oemer = _can_process_with_oemer(ext)

        if engine == "detector" and not can_use_detector:
            raise HTTPException(
                status_code=503,
                detail="Detector is not available for this upload. Ensure MODEL_WEIGHTS_PATH is set and upload a supported image.",
            )
        if engine == "oemer" and not can_use_oemer:
            raise HTTPException(
                status_code=503,
                detail="Oemer is not available for this upload. Install the oemer package and use a PDF or supported bitmap.",
            )
        ...
        if is_musicxml:
            try:
                ...
                source_label = "musicxml"
            except UnicodeDecodeError as exc:
                raise HTTPException(
                    status_code=400, detail=f"Unable to decode MusicXML file: {exc}"
                ) from exc
        else:
            ...
            if can_use_oemer and engine in {"auto", "oemer"}:
                ...
                source_label = "oemer"
            elif is_supported_image and INFERENCE_RUNNER:
                # Should not reach here because detector branch handles it,
                # but keep a guard in case logic changes.
                parsed_notes = _parse_image_with_model(tmp_file_path)
                return {
                    "filename": file.filename,
                    "bpm": bpm,
                ...
                raise HTTPException(
                    status_code=501,
                    detail="Upload a MusicXML file, a supported image type (png, jpg, jpeg, bmp, tiff), or a PDF (requires Oemer).",
                )
        ...
        if source_label:
            response_payload["source"] = source_label
...
def _can_process_with_oemer(extension: str) -> bool:
    return OEMER_RUNNER is not None and extension.lower() in OEMER_SUPPORTED_EXT


def _run_oemer(source_path: str) -> str:
    """
    Executes the Oemer runner against an uploaded file and returns the first MusicXML output.
    """
    if not OEMER_RUNNER:
        raise HTTPException(
            status_code=503,
            detail="Oemer is not installed. Install the 'oemer' package to enable PDF parsing.",
        )

    output_dir = tempfile.mkdtemp()
    try:
        try:
            OEMER_RUNNER(source_path, output_path=output_dir)
        except HTTPException:
            raise
        except Exception as exc:  # pylint: disable=broad-except
            raise HTTPException(
                status_code=502, detail=f"Oemer execution failed: {exc}"
            ) from exc

        musicxml_files = [
            os.path.join(output_dir, candidate)
            for candidate in os.listdir(output_dir)
            if candidate.lower().endswith(".musicxml")
        ]
        if not musicxml_files:
            raise HTTPException(
                status_code=502, detail="Oemer did not generate a MusicXML file."
            )

        musicxml_files.sort()
        with open(musicxml_files[0], "r", encoding="utf-8") as handle:
            return handle.read()
    finally:
